...
class PhoneBook():
    def add_account(self, name, number):
        self.name = input_list[1]
        self.number = input_list[2]
        phone_book[name] = number
        phone_book_no[number] = name
        print("The account for %s with number %011d has been added." % (name, int(number)))

    def query_no(self, name, number):
        self.number = int(input_list[1])
        self.name = phone_book_no.get(number, "Missing")
    ...
    def query_name(self, name, number):
        self.name = input_list[1]
        self.name_no = phone_book.get(name, "Missing")
...
while True:
    ...
    if command == "add":
        name = input_list[1]
        number = input_list[2]
        add_account(self, name, number)

    elif command == "query":
        number = input_list[1]
        if number.isdigit():
            query_no(self, name, number)


        else:
            query_name(self, name, number)
